"""Main ANN algorithm with convergence stopping and penalty"""
import copy
...
def ann(x, z, y, net, lamb, criterion):
    """penalty hyperparameter selection"""
    length = len(lamb)
    if length > 1:
...
def ann1(x, z, y, net, lamb, criterion):
    """penalty parameter lamb is a scalar"""
    ...
    while k < 100:
        ...
        if epoch % 10 == 0:
            ...
            if loss_cpu < loss_min*0.999:
                loss_min = copy.deepcopy(loss_cpu)
    ...
    mse, pen = criterion(output, y).tolist(), (penalty_parameters(net_cache)*lamb0).tolist()
