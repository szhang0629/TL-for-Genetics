...
class LayerC(nn.Module, ABC):
    ...
    def pen(self):
        ...
        for name, param in self.named_parameters():
            if param.requires_grad and "fc" in name:
                if name.endswith(".weight"):
                    penalty += self.bs0.pen_1d(param.reshape((-1,)))
                if name.endswith(".bias"):
                    penalty += param[0] ** 2
        return penalty
...
class LayerD(nn.Module, ABC):
    def __init__(self, n_basis_1):
        self.bs1 = Basis(n_basis_1)
        self.index = None
        torch.manual_seed(629)
        super(LayerD, self).__init__()
        self.fc0 = nn.Linear(1, self.bs1.n_basis+self.bs1.linear)
