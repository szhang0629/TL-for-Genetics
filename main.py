...
def main(seed_index, name="CHRNA5", name_data="sage", hidden_units=4):
    ...
    if not os.path.exists(path_output + str(seed_index) + ".csv"):
        ...
        df = pd.DataFrame(data={'method': ["Base"], 'pen': [0],
                                'train': [dataset_train.base(y_base, classification)],
                                'test': [dataset_test.base(y_base, classification)]})
        ...
        net_lm, lamb = net.ann(dataset_train, lamb_hyper * 100, criterion)
    ...
    net_nn, lamb = net.ann(dataset_train, lamb_hyper, criterion)
    ...
    net_ml, lamb = net.ann(dataset_train, lamb_hyper, criterion, dataset_old)
    loss_train = criterion(net_ml(dataset_train), dataset_train.y).tolist()
    loss_test = criterion(net_ml(dataset_test), dataset_test.y).tolist()
    df = df.append(pd.DataFrame(data={'method': ["ML"], 'pen': [lamb], 'train': [loss_train], 'test': [loss_test]}))
